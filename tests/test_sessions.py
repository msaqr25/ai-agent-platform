...
async def test_create_session(client: AsyncClient) -> None:
    ...
    response = await client.post("/sessions/", json={"agent_id": agent_id})
...
async def test_create_session_invalid_agent(client: AsyncClient) -> None:
    response = await client.post("/sessions/", json={"agent_id": 9999})
...
async def test_list_sessions_for_agent(client: AsyncClient) -> None:
    ...
    await client.post("/sessions/", json={"agent_id": agent_id})
    response = await client.get(f"/sessions/agent/{agent_id}")
